...
interface SidebarProps {
    currentView: 'dashboard' | 'analytics' | 'reports' | 'alerts' | 'settings';
    onViewChange: (view: 'dashboard' | 'analytics' | 'reports' | 'alerts' | 'settings') => void;
}
    ...
    return (
        <div className={`flex flex-col items-center py-6 bg-white border-r border-slate-200 z-10 transition-all duration-300 ${isExpanded ? 'w-48' : 'w-16 lg:w-20'}`}>
            ...
            <nav className="flex-1 flex flex-col gap-4 w-full px-2">
                ...
                <NavItem icon={<SettingsIcon />} label="Settings" expanded={isExpanded} onClick={() => onViewChange('settings')} />
            </nav>
